...
function StockChart({ stock }) {
    ...
    useEffect(() => {
        const chartDataRes = stock.historical;
        setArray(chartDataRes);
        setFirstDataPointPrice(chartDataRes[0].price);
        const lastPrice = chartDataRes.at(-1).price;
        const firstPrice = chartDataRes[0].price;
        const priceVariationVar = (Number(lastPrice - firstPrice).toFixed(2) / firstPrice) * 100;
        setPriceVariation(priceVariationVar.toFixed(2));
        setChartData({
            labels: chartDataRes.map(day => new Date(day.date)),
            datasets: [
                {
                    data: chartDataRes.map(day => day.price),
                    backgroundColor: (context) => {
                        const ctx = context.chart.ctx;
                        const gradient = ctx.createLinearGradient(0, 0, 0, 150);
                        gradient.addColorStop(0, chartDataRes[0].price < stock.price ? "rgba(6, 196, 47, 0.3)" : "rgba(255, 0, 0, 0.3)");
                        gradient.addColorStop(1, chartDataRes[0].price < stock.price ? "rgba(6, 196, 47, 0.01)" : "rgba(255, 0, 0, 0.01)");
                        return gradient;
                    },
                    borderColor: chartDataRes[0].price < stock.price ? "#06c42f" : "red",
                    borderWidth: 2,
                    fill: true,
                    pointRadius: 0,
                    tension: 0.3,
    ...
    const handleHover = (event, chartElements) => {
        if (chartElements.length && !shouldAnimate) {
            ...
            const hoveredPrice = hoveredData.price;
        ...
        return (
            <div className="flex-1 flex flex-col items-start w-full xl:w-auto">
                <div className="relative h-72 pt-16 w-full xl:max-w-[584px]">
                    <Line data={chartData} options={options} plugins={[maxpoint, caretHover]} />
                    <div className="absolute top-0 left-full -translate-x-full flex items-center gap-4 origin-left">
                        ...
                        <p className="font-bold">
                            {hoveredData ? formatCash(hoveredData.price, undefined, "$") : sortBy}
                        </p>
                        {
                            priceVariation ?
                                percentageBubble(priceVariation) :
                                null
                        }
                    </div>
                </div>
